import type { CalculatorState, Operation } from './types.js';
import {
    addCharToInput,
    performOperation,
    clearState,
...
let currentState: CalculatorState = {
    display: '0',
    currentInput: '0',
    previousInput: '',
    operation: null,
...
let currentMode: 'full' | 'basic' = 'basic'; // по умолчанию
...
const errorGif = document.querySelector('.error-gif') as HTMLImageElement;
    ...
    try {
        ...
        if (action === 'clear') {
            ...
            if (action === '.') {
                ...
                newState = setOperation(newState, action as Operation);
            }
...
function setupBackButtons() {
    ...
    if (backButton1) backButton1.addEventListener('click', goBack);
...
document.addEventListener('keydown', (e) => {
    ...
    if (currentMode === 'full' || currentMode === 'basic') {
        if (e.key === 'Enter') {
            ...
            if (currentMode === 'full') handleButtonClick('^');
        } else if (e.key === 'r' || e.key === 'R') {
            if (currentMode === 'full') handleButtonClick('sqrt');
        }
    }
});